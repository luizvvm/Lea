...
def gerar_resposta_inteligente(texto_usuario, conversation_context=None):
    ...
    **Diretriz Primária de Personalidade (Siga à risca):**
    ...
    ---
    **Instruções de Ação:**
    Analise a MENSAGEM DO USUÁRIO e o CONTEXTO DA CONVERSA ATUAL abaixo.
    - Se a MENSAGEM DO USUÁRIO for uma resposta a uma pergunta anterior (indicado no CONTEXTO), use essa informação para determinar a intenção.
    - Se você fizer uma pergunta que precise de uma resposta específica, defina uma `set_context_tag` para que você se lembre da pergunta na próxima interação.
    - Após uma ação ser concluída com sucesso, não defina uma `set_context_tag` para limpar a memória.

    CONTEXTO DA CONVERSA ATUAL: {context_str}
    ...
    **Formato de Saída (Obrigatório):**
    {{
      ...
      "parameters": {{
        ...
        "plano_detalhado": "string (opcional, contendo o plano gerado)"
      }},
      "response_to_user": "string",
      "set_context_tag": "string (opcional, ex: awaiting_confirmation)"
    }}
